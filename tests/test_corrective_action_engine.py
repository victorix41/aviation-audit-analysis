...
def test_corrective_action_counts() -> None:
    ...
    result = generate_corrective_action_analysis(dataframe)
...
def test_corrective_action_pareto() -> None:
    ...
    assert result.top_corrective_action_frequency == 2

    assert result.top_corrective_action_percentage == 28.57
...
def test_empty_dataframe() -> None:
    dataframe = pd.DataFrame(
        {
            "corrective_action": pd.Series(dtype="string"),
    ...
    assert result.top_corrective_action_frequency == 0

    assert result.top_corrective_action_percentage == 0.0
...
def test_missing_required_column_raises_error() -> None:
    ...
    with pytest.raises(
        ...
        generate_corrective_action_analysis(dataframe)
