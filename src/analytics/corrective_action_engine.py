...
def generate_corrective_action_analysis(
    ...
    *,
    ...
    corrective_action_series = standardise_text_series(
        dataframe[CORRECTIVE_ACTION_COLUMN],
        unspecified_label=UNSPECIFIED_LABEL,
    )

    unspecified_findings = int(corrective_action_series.eq(UNSPECIFIED_LABEL).sum())

    specified_findings = total_findings - unspecified_findings

    specified_series = corrective_action_series[
        corrective_action_series.ne(UNSPECIFIED_LABEL)
    ]

    unique_corrective_actions = int(specified_series.nunique())

    pareto_dataframe = dataframe.copy()

    pareto_dataframe[CORRECTIVE_ACTION_COLUMN] = corrective_action_series
    ...
    if pareto_result.table.empty:
        ...
        top_corrective_action = str(top_row["category"])

        top_corrective_action_frequency = int(top_row["frequency"])

        top_corrective_action_percentage = float(top_row["percentage"])
    ...
    return CorrectiveActionAnalysis(
        ...
        unique_corrective_actions=(unique_corrective_actions),
        top_corrective_action=(top_corrective_action),
        top_corrective_action_frequency=(top_corrective_action_frequency),
        top_corrective_action_percentage=(top_corrective_action_percentage),
