...
def generate_preventive_action_analysis(
    ...
    *,
    ...
    preventive_action_series = standardise_text_series(
        dataframe[PREVENTIVE_ACTION_COLUMN],
        unspecified_label=UNSPECIFIED_LABEL,
    )

    unspecified_findings = int(preventive_action_series.eq(UNSPECIFIED_LABEL).sum())

    specified_findings = total_findings - unspecified_findings

    specified_series = preventive_action_series[
        preventive_action_series.ne(UNSPECIFIED_LABEL)
    ]

    unique_preventive_actions = int(specified_series.nunique())

    pareto_dataframe = dataframe.copy()

    pareto_dataframe[PREVENTIVE_ACTION_COLUMN] = preventive_action_series
    ...
    if pareto_result.table.empty:
        ...
        top_preventive_action = str(top_row["category"])

        top_preventive_action_frequency = int(top_row["frequency"])

        top_preventive_action_percentage = float(top_row["percentage"])
    ...
    return PreventiveActionAnalysis(
        ...
        unique_preventive_actions=(unique_preventive_actions),
        top_preventive_action=(top_preventive_action),
        top_preventive_action_frequency=(top_preventive_action_frequency),
        top_preventive_action_percentage=(top_preventive_action_percentage),
