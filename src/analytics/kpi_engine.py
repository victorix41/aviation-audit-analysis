...
def generate_audit_summary(
    ...
    *,
    ...
    severity_series = (
        dataframe[SEVERITY_COLUMN].astype("string").str.strip().str.title()
    )
    ...
    recognised_severity_count = observation_count + minor_count + major_count

    unspecified_severity_count = total_findings - recognised_severity_count
    ...
    missing_due_date_count = int(due_dates.isna().sum())

    effective_timestamp = pd.Timestamp(effective_date)

    next_30_days_timestamp = effective_timestamp + pd.Timedelta(days=30)

    valid_due_dates = due_dates.dropna()

    past_due_response_count = int((valid_due_dates < effective_timestamp).sum())

    due_within_30_days_count = int(
        (
            (valid_due_dates >= effective_timestamp)
            & (valid_due_dates <= next_30_days_timestamp)
        ).sum()
    )

    future_due_count = int((valid_due_dates > next_30_days_timestamp).sum())

    if valid_due_dates.empty:
        earliest_due_date = None
        latest_due_date = None
    else:
        earliest_due_date = valid_due_dates.min().date()

        latest_due_date = valid_due_dates.max().date()
    ...
    return AuditSummary(
        ...
        unspecified_severity_count=(unspecified_severity_count),
        ...
        past_due_response_count=(past_due_response_count),
        due_within_30_days_count=(due_within_30_days_count),
