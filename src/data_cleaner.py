...
def clean_audit_data(
    ...
    cleaned_dataframe = cleaned_dataframe.rename(columns=COLUMN_RENAME_MAP)

    cleaned_dataframe = clean_text_columns(cleaned_dataframe)

    if "severity_level" in cleaned_dataframe.columns:
        cleaned_dataframe["severity_level"] = cleaned_dataframe[
            "severity_level"
        ].str.title()
